...
dir = 'Krauss'
model_type = 'lstm'
model_id = f'h_{HIDDEN_SIZE}_l_{NUM_LAYERS}_lr_{LEARNING_RATE}_lag_{NUM_LAGS}'
...
with mlflow.start_run(run_name=f"{model_type}_{model_id}") as parent_run:
    ...
    class LSTMModel(nn.Module):
        def __init__(self, input_size=1, hidden_size=HIDDEN_SIZE, num_classes=2, num_layers=NUM_LAYERS, dropout=DROPOUT):
    ...
    for year in range(START_Y, END_Y + 1):
        with mlflow.start_run(run_name=f"year_{year}", nested=True):
            sequences_df = pd.read_parquet(f'{dir}/input/lag_{NUM_LAGS}/returns_{year}.parquet')
